...
{
    public partial class Foodsubgroupinsert : Form
    {
        ...
        private void Foodsubgroupinsert_Load(object sender, EventArgs e)
        {
            CenterLabel();
        ...
        private void Foodsubgroupinsert_Resize(object sender, EventArgs e)
        {
            if (resizer != null)
            {
                resizer.ResizeControls(this);
        ...
        private void InsertDataGridViewData()
        {
            ...
            try
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    if (!int.TryParse(row.Cells[0].Value.ToString(), out int subgroupId))
                    {
                    ...
                    string subgroupName = row.Cells[1].Value?.ToString() ?? "";

                    if (!int.TryParse(row.Cells[2].Value.ToString(), out int groupId))
                    {
                    ...
                    string query = "INSERT INTO Subgroups (SubgroupID, SubgroupName, GroupID) VALUES (@SubgroupID, @SubgroupName, @GroupID);";
        ...
        {
            this.Hide();
            Foodsubsubgroupinsert foodsubsubgroupinsert = new Foodsubsubgroupinsert();
            foodsubsubgroupinsert.ShowDialog();
        }
        ...
        {
            string query = "DELETE FROM Subgroups;";
        ...
        public void LocalizeControls()
        {
            ...
            label4.Text = LanguageManager.Instance.GetString("FSGI-lbl4");
