...
{
    public partial class Modifysubgroup : UserControl
    {
        ...
        public void LoadComboBoxes(ComboBox customBox1)
        {
            try
            {
                ...
                customBox1.DataSource = crudDatabase.Groups;
                customBox1.DisplayMember = "GroupName";
                customBox1.ValueMember = "GroupID";
        ...
        {
            string query = "SELECT * FROM Subgroups";
        ...
        private void roundedTextbox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (dataTable != null)
            {
                ...
                {
                    dataView.RowFilter = $"SubgroupName LIKE '%{filterExpression}%'";
                }
                else
                {
            ...
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == dataGridView1.Columns["Update"].Index)
                {
                    ...
                    if (selectedId != null)
                    {
                        ...
                        LoadSubgroupDetails(selectedId.ToString());
        ...
        private void LoadSubgroupDetails(string subgroupId)
        {
            DataTable subgroupDetails = crudDatabase.FetchDataFromDatabase($"SELECT SubgroupID, SubgroupName, GroupID FROM Subgroups WHERE SubgroupID = '{subgroupId}'");

            if (subgroupDetails.Rows.Count > 0)
            {
                roundedTextbox2.Text = subgroupDetails.Rows[0]["SubgroupID"]?.ToString() ?? "N/A";
                roundedTextbox3.Text = subgroupDetails.Rows[0]["SubgroupName"]?.ToString() ?? "N/A";

                // Fetch the corresponding GroupName using the GroupID
                int groupId = Convert.ToInt32(subgroupDetails.Rows[0]["GroupID"]);
                customBox1.SelectedValue = groupId;
        ...
        public void InsertSubgroup()
        {
            ...
            {
                label18.Text = "Please enter a Subgroup name.";
                label18.ForeColor = Color.Tomato;
                return;
            }
            ...
            {
                label18.Text = "Please select a parent group for your subgroup.";
                label18.ForeColor = Color.Tomato;
                return;
            }
            ...
            if (customBox1.SelectedItem is CrudDatabase.Group selectedGroup)
            {
                int groupId = selectedGroup.GroupID;

                // Prepare the insert query
                string insertQuery = "INSERT INTO Subgroups (SubgroupName, GroupID) VALUES (@name, @groupId)";
                ...
                if (crudDatabase.ExecuteNonQuery(insertQuery, parameters))
                {
                    clearControls();
                    LoadSubgroupsTable();
                    label18.Text = "Subgroup inserted successfully!";
                    label18.ForeColor = Color.LawnGreen;
                    this.Alert("Subgroup added. Success!", Alertform.enmType.Success);
                }
                else
                {
        ...
        public void UpdateSubgroup()
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (selectedSubgroupId != null && int.TryParse(selectedSubgroupId.ToString(), out int subgroupId))
                {
                    ...
                    if (customBox1.SelectedItem is CrudDatabase.Group selectedGroup)
                    {
                        int groupId = selectedGroup.GroupID; // Get the GroupID from the selected group

                        // Prepare the update query
                        string updateQuery = "UPDATE Subgroups SET SubgroupName = @name, GroupID = @groupId WHERE SubgroupID = @id";
                        var parameters = new Dictionary<string, object>
                        {
                            { "@id", subgroupId },
                            { "@name", roundedTextbox3.Text },
                        ...
                        if (crudDatabase.ExecuteNonQuery(updateQuery, parameters))
                        {
                            ...
                            roundedPanel3.Visible = false;                           
                            label18.ForeColor = Color.LawnGreen;
                            label18.Text = "Subgroup updated successfully!";
                            rjButton1.BackColor = SystemColors.Highlight;
                            this.Alert("Subgroup updated successfully!", Alertform.enmType.Success);
                        }
                        else
                        {
        ...
        public void DeleteSubgroup()
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (selectedId != null && int.TryParse(selectedId.ToString(), out int groupId))
                {
                    ...
                    string deleteQuery = "DELETE FROM Subgroups WHERE SubgroupID = @id";
                    var parameters = new Dictionary<string, object>
                    {
                        { "@id", groupId }
                    };
