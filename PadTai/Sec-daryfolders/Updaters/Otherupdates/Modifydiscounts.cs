...
{
    public partial class Modifydiscounts : UserControl
    {
        ...
        private void roundedTextbox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (dataTable != null)
            {
                ...
                {
                    dataView.RowFilter = $"Thediscount LIKE '%{filterExpression}%'";
                }
                else
                {
        ...
        private void LoadGroupDetails(string discountId)
        {
            string query = $"SELECT DiscountID, Thediscount FROM Discounts WHERE DiscountID = '{discountId}'"; ;

            DataTable discountDetails = crudDatabase.FetchDataFromDatabase(query);

            if (discountDetails.Rows.Count > 0)
            {
                roundedTextbox2.Text = discountDetails.Rows[0]["DiscountID"]?.ToString() ?? "N/A";
                roundedTextbox3.Text = discountDetails.Rows[0]["Thediscount"]?.ToString() ?? "N/A";
            }
            else
            {
        ...
        public void InsertGroup()
        {
            ...
            insertQuery = "INSERT INTO Discounts (Thediscount) VALUES (@name)";
            ...
            {
                this.Alert("Discount added successfully!", Alertform.enmType.Success);
                label18.Text = "Discount inserted successfully!";
            ...
            {
                label18.Text = "Failed to insert Discount.";
                label18.ForeColor = Color.Tomato;
            }
        }
        ...
        public void UpdateGroup()
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (selectedId != null && int.TryParse(selectedId.ToString(), out int groupId))
                {
                    ...
                    string updateQuery = "UPDATE Discounts SET Thediscount = @name WHERE DiscountID = @id";
                    var parameters = new Dictionary<string, object>
                    {
                        { "@id", groupId },
                        { "@name", roundedTextbox3.Text }
        ...
        public void DeleteDiscount()
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (selectedId != null && int.TryParse(selectedId.ToString(), out int groupId))
                {
                    ...
                    string deleteQuery = "DELETE FROM Discounts WHERE DiscountID = @id";
                    var parameters = new Dictionary<string, object>
                    {
                        { "@id", groupId }
                    };
