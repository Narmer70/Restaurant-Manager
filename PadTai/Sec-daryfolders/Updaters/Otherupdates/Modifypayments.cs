...
{
    public partial class Modifypayments : UserControl
    {
        ...
        public void LoadComboBoxes(ComboBox customBox1)
        {
            try
            {
                crudDatabase.LoadPaymentGroups();
 
                var paymentGroupsWithDefault = new List<CrudDatabase.PaymentGroups>
                {
                     new CrudDatabase.PaymentGroups { PaymentgroupID = -1, PaymentGroupName = "Null" } 
                };

                paymentGroupsWithDefault.AddRange(crudDatabase.Paymentgroups);
                customBox1.DataSource = paymentGroupsWithDefault;
                customBox1.DisplayMember = "PaymentGroupName";
                customBox1.ValueMember = "PaymentgroupID";
        ...
        private void roundedTextbox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (dataTable != null)
            {
                ...
                {
                    dataView.RowFilter = $"PaymenttypeName LIKE '%{filterExpression}%'";
                }
                else
                {
        ...
        private void LoadSubsubgroupDetails(string subsubgroupId)
        {
            try
            {
                DataTable subsubgroupDetails = crudDatabase.FetchDataFromDatabase($"SELECT PaymentypeID, PaymenttypeName, PaymentgroupID FROM PaymentTypes WHERE PaymentypeID = '{subsubgroupId}'");

                if (subsubgroupDetails.Rows.Count > 0)
                {
                    roundedTextbox2.Text = subsubgroupDetails.Rows[0]["PaymentypeID"]?.ToString() ?? "N/A";
                    roundedTextbox3.Text = subsubgroupDetails.Rows[0]["PaymenttypeName"]?.ToString() ?? "N/A";

                    // Check if PaymentgroupID is DBNull
                    if (subsubgroupDetails.Rows[0]["PaymentgroupID"] != DBNull.Value)
                    {
                        int subgroupId = Convert.ToInt32(subsubgroupDetails.Rows[0]["PaymentgroupID"]);
                        customBox1.SelectedValue = subgroupId;
                    }
                    else
                    {
                        customBox1.SelectedValue = -1;
                        customBox1.Text = "No Payment group assigned.";
                    }
                }
                else
                {
        ...
        public void InsertSubsubgroup()
        {
            try
            {
                // Validate Subsubgroup name input
                ...
                string insertQuery = "INSERT INTO PaymentTypes (PaymenttypeName, PaymentgroupID) VALUES (@name, @groupId)";

                // Prepare parameters for the insert
                var parameters = new Dictionary<string, object>
                {
                      { "@name", roundedTextbox3.Text },
                      { "@groupId", DBNull.Value } // Default to NULL
                };

                // Check if a group is selected and update the groupId parameter
                if (customBox1.SelectedItem is CrudDatabase.PaymentGroups selectedGroup)
                {
                    int groupId = selectedGroup.PaymentgroupID;
            ...
            try
            {
                ...
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    if (selectedSubgroupId != null && int.TryParse(selectedSubgroupId.ToString(), out int subgroupId))
                    {
                        ...
                        string updateQuery = "UPDATE PaymentTypes SET PaymenttypeName = @name, PaymentgroupID = @groupId WHERE PaymentypeID = @id";
                        var parameters = new Dictionary<string, object>
                        {
                             { "@id", subgroupId },
                             { "@name", roundedTextbox3.Text },
                        ...
                        // If the default item is selected, set groupId to DBNull
                        if (customBox1.SelectedValue != null && (int)customBox1.SelectedValue == -1)
                        {
                            parameters["@groupId"] = DBNull.Value; // Set to NULL
                        }
                        else if (customBox1.SelectedItem is CrudDatabase.PaymentGroups selectedGroup)
                        {
                            int groupId = selectedGroup.PaymentgroupID; 
                            parameters["@groupId"] = groupId; 
                        }
                        ...
                        {
                            parameters["@groupId"] = DBNull.Value; 
                        }
            ...
            try
            {
                ...
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    if (selectedId != null && int.TryParse(selectedId.ToString(), out int groupId))
                    {
                        ...
                        string deleteQuery = "DELETE FROM PaymentTypes WHERE PaymentypeID = @id";
                        var parameters = new Dictionary<string, object>
                    {
                        { "@id", groupId }
                    };
