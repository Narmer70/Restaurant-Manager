...
{
    public partial class Paytypereport : UserControl
    {
        ...
        private void LoadPaymentTypes()
        {
            ...
            {
                connection.Open();
                SqlCommand cmd = new SqlCommand("SELECT DISTINCT PaymentTypeName FROM Receipts " +
                ...
                while (reader.Read())
                {
                    comboBox1.Items.Add(reader["PaymentTypeName"].ToString());
                }
            }
        }
        ...
        private void LoadReceipts(string paymentType)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT ReceiptId, FoodName, TotalPrice FROM Receipts " +
                    "WHERE PaymentTypeName = @PaymentTypeName AND ClientID = @ClientID", connection);

                cmd.Parameters.AddWithValue("@PaymentTypeName", paymentType);
                cmd.Parameters.AddWithValue("@ClientID", clientId);
