...
namespace PadTai.Sec_daryfolders.Quitfolder
{
    public partial class Paytypecatreport : UserControl
    {
        ...
        public void LoadPaymentTypeSalesReport()
        {
            ...
            string query = @"SELECT PaymenttypeName, COUNT(*) AS Occurrences, SUM(TotalPrice) AS TotalPriceSum
                       FROM Receipts WHERE ClientID = @ClientID GROUP BY PaymenttypeName ORDER BY PaymenttypeName";
            ...
            try
            {
                DataTable paymentTypeSalesReportData = crudDatabase.FetchDataFromDatabase(query, parameters);

                if (dataGridView1.Columns.Count == 0)
                {
                    dataGridView1.Columns.Add("PaymentType", LanguageManager.Instance.GetString("Dailyreport-paytype"));
                    dataGridView1.Columns.Add("Occurrences", LanguageManager.Instance.GetString("Dailyreport-qte"));
                    dataGridView1.Columns.Add("TotalPrice", LanguageManager.Instance.GetString("Dailyreport-sum"));
                }

                decimal totalOverallAmount = 0m;
                int totalReceiptCount = 0;

                foreach (DataRow row in paymentTypeSalesReportData.Rows)
                {
                    string paymentTypeValue = row["PaymenttypeName"].ToString();
                    int occurrencesValue = Convert.ToInt32(row["Occurrences"]);
                    decimal priceValue = Convert.ToDecimal(row["TotalPriceSum"]);

                    string paymentTypespaced = "     " + paymentTypeValue;
                    string priceValuespaced = priceValue.ToString("N2") + $" {CurrencyService.Instance.GetCurrencySymbol()}";

                    dataGridView1.Rows.Add(paymentTypespaced, occurrencesValue, priceValuespaced);

                    totalOverallAmount += priceValue;
                    totalReceiptCount += occurrencesValue;
                }

                label1.Text = totalReceiptCount.ToString();
                label2.Text = totalOverallAmount.ToString("N2") + $" {CurrencyService.Instance.GetCurrencySymbol()}";
            }
            catch
            {
            }
        ...
        public void LoadReceipts(string paymentType)
        {
            ...
            string query = "SELECT ReceiptId, FoodName, TotalPrice FROM Receipts WHERE PaymentTypeName = @PaymentTypeName AND ClientID = @ClientID";
            var parameters = new Dictionary<string, object>
            {
        ...
        private void ConfigureDataGridView5()
        {
