...
{
    public partial class Dailydeliverychart : UserControl
    {
        private Dictionary<string, Dictionary<int, int>> paymentTypeData;
        private Dictionary<string, int> deliveryData;
        private Random random = new Random();
        private CrudDatabase crudDatabase;
        private FontResizer fontResizer;
        private ControlResizer resizer;

        public Dailydeliverychart()
        {
            InitializeComponent();
        ...
        private void LoadDonutChart()
        {
            ...
            string query = @"SELECT PlacetoEatName, COUNT(ReceiptID) AS OccurrenceCount
                 FROM Receipts WHERE ClientID = @ClientID GROUP BY PlacetoEatName;";
            ...
            try
            {
                DataTable deliveryDataQueryResult = crudDatabase.FetchDataFromDatabase(query, parameters);

                deliveryData = new Dictionary<string, int>();

                foreach (DataRow row in deliveryDataQueryResult.Rows)
                {
                    string deliveryDataName = row["PlacetoEatName"].ToString();

                    if (row["OccurrenceCount"] != DBNull.Value)
                    {
                        int occurrenceCount = Convert.ToInt32(row["OccurrenceCount"]);
                        deliveryData[deliveryDataName] = occurrenceCount;
                    }
                }
            }
            ...
            double totalCount = deliveryData.Values.Sum();

            foreach (var deliveryData in deliveryData)
            {
                donutSeries.Points.AddXY(deliveryData.Key, deliveryData.Value);
                ...
                double percentage = (double)deliveryData.Value / totalCount * 100;
                donutSeries.Points[donutSeries.Points.Count - 1].Label = $"{deliveryData.Value}";
                donutSeries.Points[donutSeries.Points.Count - 1].LegendText = $"{deliveryData.Key}: {deliveryData.Value} ({percentage:N1}%)";
        ...
        private void LoadLineChart()
        {
            ...
            string query = @"SELECT strftime('%H', OrderDateTime) AS Hour, COUNT(ReceiptID) AS OccurrenceCount, 
                 PlacetoEatName FROM Receipts WHERE ClientID = @ClientID GROUP BY Hour, PlacetoEatName ORDER BY Hour;";
            ...
            try
            {
                DataTable paymentTypeDataQueryResult = crudDatabase.FetchDataFromDatabase(query, parameters);

                paymentTypeData = new Dictionary<string, Dictionary<int, int>>();

                foreach (DataRow row in paymentTypeDataQueryResult.Rows)
                {
                    string paymentType = row["PlacetoEatName"].ToString();
        ...
        public void LocalizeControls()
        {
            ...
            title1.Text = LanguageManager.Instance.GetString("Dcharts-deliveryt1");

            var title2 = chart2.Titles[0]; 
            title2.Text = LanguageManager.Instance.GetString("Dcharts-deliveryt2"); 
