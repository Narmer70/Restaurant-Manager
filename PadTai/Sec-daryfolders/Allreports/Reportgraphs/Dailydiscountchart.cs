...
{
    public partial class Dailydiscountchart : UserControl
    {
        private Dictionary<string, int> discounttypes;
        private Random random = new Random();
        private CrudDatabase crudDatabase;  
        private FontResizer fontResizer;
        private ControlResizer resizer;

        public Dailydiscountchart()
        {
            InitializeComponent();
        ...
        private void LoadDonutChart()
        {
            ...
            // First code snippet
            string query = @"SELECT Thediscount, COUNT(ReceiptID) AS OccurrenceCount
                 FROM Receipts WHERE ClientID = @ClientID GROUP BY Thediscount;";
            ...
            try
            {
                DataTable discountData = crudDatabase.FetchDataFromDatabase(query, parameters);
                discounttypes = new Dictionary<string, int>();

                foreach (DataRow row in discountData.Rows)
                {
                    string discountName = row["Thediscount"].ToString();

                    if (row["OccurrenceCount"] != DBNull.Value)
                    {
                        int occurrenceCount = Convert.ToInt32(row["OccurrenceCount"]);
                        discounttypes[discountName] = occurrenceCount;
                    }
                }
            }
            ...
            double totalCount = discounttypes.Values.Sum();

            foreach (var paymentType in discounttypes)
            {
        ...
        private void LoadLineChart()
        {
            ...
            string query = @"SELECT strftime('%H', OrderDateTime) AS Hour, COUNT(ReceiptID) AS OccurrenceCount, 
                 Thediscount FROM Receipts WHERE ClientID = @ClientID GROUP BY Hour, Thediscount ORDER BY Hour;";
            ...
            try
            {
                ...
                {
                    string discountType = row["Thediscount"].ToString();
                    int hour = Convert.ToInt32(row["Hour"]);
                    int occurrenceCount = Convert.ToInt32(row["OccurrenceCount"]);

                    if (!paymentTypeData.ContainsKey(discountType))
                    {
                        paymentTypeData[discountType] = new Dictionary<int, int>();
                    }

                    paymentTypeData[discountType][hour] = occurrenceCount;
                }
            }
            catch (SqlException Sqlex)
            {
        ...
        public void LocalizeControls()
        {
            ...
            title1.Text = LanguageManager.Instance.GetString("Dcharts-discountt1");

            var title2 = chart2.Titles[0];
            title2.Text = LanguageManager.Instance.GetString("Dcharts-discountt2");
